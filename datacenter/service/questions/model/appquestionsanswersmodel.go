...
type (
	AppQuestionsAnswersModel interface {
		...
		FindOne(id int64) (*AppQuestionsAnswers, error)
...
func (m *defaultAppQuestionsAnswersModel) Insert(data AppQuestionsAnswers) (sql.Result, error) {
	...
	ret, err := m.ExecNoCache(query, data.Beid, data.Ptyid, data.Uid, data.Auid, data.TestId, data.ActivityId, data.Answers, data.Score)

	return ret, err
}

func (m *defaultAppQuestionsAnswersModel) FindOne(id int64) (*AppQuestionsAnswers, error) {
...
func (m *defaultAppQuestionsAnswersModel) Update(data AppQuestionsAnswers) error {
	...
	_, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.Exec(query, data.Beid, data.Ptyid, data.Uid, data.Auid, data.TestId, data.ActivityId, data.Answers, data.Score, data.Id)
